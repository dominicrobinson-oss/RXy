export default function MegaMenuSupplements() {
  return (
    <div className="absolute left-0 top-full w-screen bg-white border-t border-gray-200 shadow-lg animate-slideDown z-50">
      <div className="mx-auto max-w-7xl px-8 py-6 grid grid-cols-4 gap-8">
        <div>
          <h3 className="text-sm font-semibold">Supplements</h3>
          <ul className="mt-3 space-y-2 text-sm text-gray-700">
            <li><a href="#" className="hover:text-black">Creatine</a></li>
            <li><a href="#" className="hover:text-black">Pre-workout</a></li>
          </ul>
        </div>
        <div>
          <h3 className="text-sm font-semibold">Support</h3>
          <ul className="mt-3 space-y-2 text-sm text-gray-700">
            <li><a href="#" className="hover:text-black">Hydration</a></li>
            <li><a href="#" className="hover:text-black">Sleep</a></li>
          </ul>
        </div>
        <div>
          <h3 className="text-sm font-semibold">Routine</h3>
          <ul className="mt-3 space-y-2 text-sm text-gray-700">
            <li><a href="#" className="hover:text-black">Before training</a></li>
            <li><a href="#" className="hover:text-black">After training</a></li>
          </ul>
        </div>
        <div>
          <h3 className="text-sm font-semibold">Shop all</h3>
          <ul className="mt-3 space-y-2 text-sm text-gray-700">
            <li><a href="#" className="hover:text-black">All supplements</a></li>
          </ul>
        </div>
      </div>
    </div>
  )
}
